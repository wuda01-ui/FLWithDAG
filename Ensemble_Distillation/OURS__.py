...
def kd_train(synthesizer, model, criterion, optimizer, device):
    ...
    with tqdm(synthesizer.get_data()) as epochs:
        for idx, (images) in enumerate(epochs):
            ...
            loss_ensem = criterion(s_out, t_out.detach())

            loss_feat = 0.001*torch.stack(
                        [criterion_feat(s_feat, t_feat.detach()) for s_feat, t_feat in zip(s_feats, t_feats)]).mean()
            loss_mh = torch.stack(
                    [criterion(s_logit, t_logit.detach()) for s_logit, t_logit in zip(s_outs, t_outs)]).mean()  
...
def mhtest(model, test_loader, device):
    device = device
    model.eval()
    test_loss = 0
    correct = 0
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        acc, test_loss = mhtest(global_model, test_loader, args.device)
